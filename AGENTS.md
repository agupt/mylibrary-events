...
- **Not yet solved**: Sunnyvale (CA0143) BiblioCommons RSS is empty — events only in the `v2/search` JSON API (needs a different adapter). Los Gatos (CA0164) LibCal instance has events on-page but every `rss.php?cid=` returns empty — needs the real serving calendar id.
