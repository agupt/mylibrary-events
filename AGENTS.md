...
- **Alameda Free Library (CA0002)**: `mylibrary.digital` platform behind Cloudflare's JS challenge (app 403s non-browsers) — adapter `src/lib/events/custom/myLibraryDigitalProvider.ts` (found by Amool, 2026-07-16). Escape hatch: the `/rss` endpoint is CF-exempt (like FLP). Event date/time lives in the description HTML ("Date/Time: Fri, 17 Jul 2026, 10:30am - 11:00am"), NOT pubDate; no branch field → main outlet. Subdomain-per-library (`<slug>.events.mylibrary.digital`), reusable. The POST `/update` API needs a per-browser `cf_clearance` cookie — don't bother, use /rss.
- **Menlo Park (CA0067)**: OpenCities — adapter `src/lib/events/custom/openCitiesProvider.ts`. POST `/ocapi/calendars/getcalendaritems` {Ids,StartDate,EndDate}; libraries split into audience calendars, target the children's GUID (stored as `?calendars=` on the registry URL).
- **Monterey County (CA0073)**: shared county "GetEventsByDay" calendar — adapter `src/lib/events/custom/govCalProvider.ts` (found by Amool, 2026-07-16). POST `/api/events/GetEventsByDay/<Instance>` with `search:"labels:MCFL"` to filter to the library department; `DateStart` is floating local (the trailing Z is bogus — `StartUTC` is the real instant), `AllDay` flag honored, branch from the "<Branch> - <Title>" name prefix. Registry URL carries `?ppid=&label=&lat=&lng=`.
- **ICS recurrence**: `parseIcs` now surfaces `rrule`/`exdates` and the ICS provider expands them via `src/lib/events/recurrence.ts` (Benicia's Solspace feed anchored weekly/monthly storytimes years back — 0 events without expansion). Applies to every ICS system.
- **Sunnyvale (CA0143)**: SOLVED — config-only, existing LibCal RSS adapter (2026-07-16). The old "events live in BiblioCommons v2/search JSON API" hypothesis was WRONG: BiblioCommons' events feature is *disabled* for Sunnyvale (`gateway.bibliocommons.com/v2/libraries/sunnyvale/events` → 403 "Events feature is not available"; the catalog SPA reports `events.enabled=false`; `/events/rss/all` is a valid but empty feed). The real source is `sunnyvale.libcal.com/rss.php?m=month&cid=13025` — the system was already registered as libcal/detected "needs calendar id". Exposed a real bug: single-outlet systems tag `libcal:campus` with a *department* ("Youth Services"), which matched no branch and so the provider's `hasCampusData && !branch → drop` guard discarded every event. Fix: `createLibcalRssProvider` now takes `outletCountForSystem`; single-outlet systems skip the campus-mismatch drop (a lone outlet has no branch ambiguity). Also filters `STAFF ONLY`-prefixed internal events like it already filters `CANCELLED`.
- **Not yet solved**: Los Gatos (CA0164) LibCal instance has events on-page but every `rss.php?cid=` returns empty — needs the real serving calendar id.
- **Redwood City (CA0101)**: CivicPlus municipal site — adapter `src/lib/events/custom/civicPlusProvider.ts` (found by Amool, 2026-07-16). Per-category RSS at `/Home/Components/RssFeeds/RssFeed/View?id=N`; enumerate ids to find the library one (RWC id=4 "Library Feed"; id=1 is the whole city). Items carry structured `<eventStartDate>M/D/YYYY h:mm:ss AM/PM</eventStartDate>` (clean, no heuristic); no age/location fields, so age is text-inferred and branch comes from an "@ &lt;Branch&gt;" title suffix. Reusable for other CivicPlus cities.
- **Keller (TX0120)**: SOLVED — **whofi** platform, adapter `src/lib/events/custom/whofiProvider.ts` (curl found by Amool, 2026-07-18). Was tagged needs-scraper (Granicus) — wrong; the real source is a clean unauthenticated JSON API at `<slug>.whofi.com/calendar/fetch_calendar_events?start=<iso>&end=<iso>` (Keller slug `keller-tx`). Forgiving: date-only params work, no tz offset, no WAF (bot UA gets full data). Only current+next month is published, so a wide window still returns whatever exists. Each event carries a **structured `age` label** ("Children (Ages 0-5)", "Young Adult (Ages 12-18)", "Adult (Ages 19+)", "General Interest") that is authoritative — 3-way age decision: numeric range → `ageGroupsFromRange`; explicit adult label → drop; empty/"General Interest" → `inferAgeGroupsFromText`. `start`/`end` are floating local wall-clock ("2026-09-05 16:00:00", no offset) so no tz projection. Branch via `location_name` → `namesOverlap` → main-fallback (Keller is single-outlet). Subdomain-per-library, so **reusable — future whofi libraries are config-only**. Verified live: 45 kid events → TX0120-002.
- **Los Angeles Public Library (CA0063)**: SOLVED — Drupal events search HTML scraper, adapter `src/lib/events/custom/laplProvider.ts` (curl found by Amool, 2026-07-18). 73 branches. No JSON/ICS surface exists (`_format=json`→406, JSON:API→404, no ICS/RSS/JSON-LD), but `/events/search` serves fine to a browser UA over plain HTTP (no Imperva/F5 wall) — so it's a **live server-side scraper, not a snapshot**. Each `c-teaser-standard--event` teaser carries title+slug, date (M/D/YYYY), time (with AM/PM — no heuristic), and branch (`__location` → `namesOverlap`, 62/73 attributed) — but **NOT audience**. So the age signal is in the QUERY, not the HTML: fetch once per kid-audience (`1555` Babies&Toddlers→baby,toddler; `1556` Kids→preschool,school-age; `2976` All Ages→all-ages), tag every result with that audience's groups, then **dedup across audiences by slug+startTime and UNION the age groups** (an event in both Babies and Kids ends up baby..school-age). **SYSTEM-WIDE by design** (not per-branch): one shared, day-quantized, SWR-cached fetch amortizes across all users — per-branch would fragment the cache and multiply against the always-in-scope rule. **Teens (`1557`) intentionally excluded** — app has no teen age group (see repeated Reddit asks; deferred). **Pagination gotcha (silent-under-fetch trap):** LAPL only renders its numbered pager when `search=/branch=/proximity[]` params are present; with a minimal URL the pager is absent but `&page=N` still works — trusting the pager silently returned 58 events instead of 379. Fix: **blind batched pagination** — page until a page returns <24 teasers, `PAGE_CONCURRENCY=5`, `MAX_PAGES_PER_AUDIENCE=15` cap (logged). Verified live: **379 kid events / 62 branches / 14d**.

Pattern for future metros: open the events page in Chrome DevTools, filter network to xhr/fetch, replicate the data call with curl, verify shape, then adapter (JSON API) or config (known vendor on odd subdomain) or needs-scraper (bot-walled SSR). For paginated HTML scrapers, NEVER trust the rendered pager — blind-paginate until a short page (a missing/quirky pager silently truncates and the wrong count looks plausible).
